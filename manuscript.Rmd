...
In the example dataset, omitting FDR correction would therefore lead to identification of 696
Reactomes with p<0.05, of which 345 are likely false positives (~49.6%).
...
When there is no background gene list present, the gene list of interest is compared to the entire genome of the organism - this is not a true representation of enrichment analysis. The meaning is lost and incorrect interpretations can be made. For example - for a pathway dysregulation in the brain of an organism, it is incorrect to compare input gene list for the entire body. It should be compared to the tissue of interest. In this case - the brain genome. 
...
AB
Methods lacking detailed information often lead to problems in reproducibility and validation. Authors should provide the list of tools used, version numbers of tools used, which options were applied for the analysis, which gene list was used, whether background gene list was applied, which statistical tests were done, what tools were used to obtain a certain result. The more descriptive the methodology is, the better for an independent researcher to go through the analysis or use the results for future experiments. 
