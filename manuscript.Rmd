...
A commonly used test statistic is the P-value. P-value refers to the probability of an observed result occurring by random chance. P value is important to accept or reject the null hypothesis (H₀) of an experiment. Null hypothesis in enrichment analysis refers to a gene (from gene list of interest) randomly occurring in a category of interest (eg - gene ontology, pathways, specific gene sets, etc). Smaller the P value, lower is the chance that the particular gene has associations randomly with the category of interest. In other words, when P value is lower, the presence of the gene is significant. 

Raw P values do not account for false positives. It is important to do P value correction to account for multiple testing and reduce the chance/probability of getting a false positive when compared to background gene list.  
P value correction is recommended before any data interpretations are made. FDR values are better values for significance comparison. 

There are certain thresholds followed by researchers to deem a term significant. P value less than 0.05 is usually used.
