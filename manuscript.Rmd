...
Of the 1840 Reactomes with five or more members detected, we obtained a mean of 51.6 hits with these
random genes (Figure 1A), proving that reporting raw p-values is bound to yield many false positives,
in this case a rate of 2.8%.

Raw P values do not account for false positives.
It is important to do P value correction to account for multiple testing and reduce the
chance/probability of getting a false positive when compared to background gene list.
P value correction is recommended before any data interpretations are made.
FDR values are better
values for significance comparison.

There are certain thresholds followed by researchers to deem a term significant.
P value less than 0.05 is usually used.
