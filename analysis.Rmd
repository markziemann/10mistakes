...
average of 51.6 pathways with p<0.01, while after p-value correction,
that number was just 0.16 with FDR<0.01.
...
boxplot(m1sim,main="Random foreground (100 repetitions)",ylab="no. gene sets meeting \"significance\" threshold <0.01",frame=FALSE)
...
par(mfrow=c(1,2))
boxplot(m1sim, ylab="no. gene sets meeting \"significance\" threshold <0.01",
  frame=FALSE, ylim=c(0,60))
beeswarm(list(mean(m1sim[,1]), mean(m1sim[,2])),add=TRUE,pch=4,cex=2)
lab1 <- paste("Mean=",signif(mean(m1sim[,1]) , 3),sep="")
lab2 <- paste("Mean=",signif(mean(m1sim[,2]) , 2),sep="")
text(c(1.2,2.2), c( mean(m1sim[,1])+5, mean(m1sim[,2])+5 ), labels=c(lab1,lab2) )
mtext("Random foreground\nReactome gene sets\n100 repetitions")
dev.off()
...
Now repeat with simulated gene sets.
We only include genes already in Reactome - this mostly gets rid of the redundant gene set question
The size of gene sets is same as those in reactome.

```{r,mistake1_sim2,fig.height=6,fig.width=6}

#all_genes <- unique(sapply(strsplit(rownames(mx)," "),"[[",2))
all_genes <- unique(unlist(gsets))

gslengths <- unlist(lapply(gsets,length))
...
m2sim <- mclapply(seeds, function(seed) {
  ...
  gsrand <- lapply(1:length(gslengths),function(i) {
    gslen <- gslengths[i]
    set.seed(seed * i )
    gs <- sample(x=all_genes,size=gslen, replace=FALSE)
    return(gs)
  } )
  names(gsrand) <- paste("gsrand",1:length(gsrand))
  fres_rand <- fora(pathways=gsrand, genes=rand, universe=bg, minSize = 5, maxSize = Inf)
  nom <- nrow(subset(fres_rand,pval<0.01))
  fdr <- nrow(subset(fres_rand,padj<0.01))
  c(nom,fdr)
},mc.cores=8)
...
colnames(m2sim) <- c("p<0.01","FDR<0.01")

boxplot(m2sim,main="Random foreground and gene sets (100 repetitions)",ylab="no. gene sets meeting \"significance\" threshold <0.01",frame=FALSE)
...
text(c(1.2,2.2), c( mean(m2sim[,1])+5, mean(m2sim[,2])+5 ), labels=c(lab1,lab2) )
...
boxplot(m2sim, ylab="no. gene sets meeting \"significance\" threshold <0.01",
  frame=FALSE,ylim=c(0,60))
beeswarm(list(mean(m2sim[,1]), mean(m2sim[,2])),add=TRUE,pch=4,cex=2)
lab1 <- paste("Mean=",signif(mean(m2sim[,1]) , 3),sep="")
lab2 <- paste("Mean=",signif(mean(m2sim[,2]) , 2),sep="")
...
unlink("img/1ax.png")
png("img/1a.png", width = 560, height = 480)
par(mfrow=c(1,2))
boxplot(m1sim, ylab="no. gene sets meeting \"significance\" threshold <0.01",
  frame=FALSE, ylim=c(0,60))
beeswarm(list(mean(m1sim[,1]), mean(m1sim[,2])),add=TRUE,pch=4,cex=2)
...
unlink("img/1ax.pdf")
pdf("img/1a.pdf", width = 6, height = 5)
par(mfrow=c(1,2))
boxplot(m1sim, ylab="no. gene sets meeting \"significance\" threshold <0.01",
  frame=FALSE,ylim=c(0,60))
beeswarm(list(mean(m1sim[,1]), mean(m1sim[,2])),add=TRUE,pch=4,cex=2)
...
gsetsf <- gsets[which(lapply(gsets,length)>5)]
commonality_score <- unlist(mclapply( gsetsf, function(i) {
  jac <- unlist(lapply( gsets,function(j) {
    myintx <- length(intersect(i,j))
    myunion <- length(union(i,j))
    myintx/myunion
  } ))
  jac <- jac[order(-jac)]
  mean(jac[2:4])
...
names(commonality_score) <- names(gsetsf)

hist(commonality_score)

gslengths <- unlist(lapply(gsetsf,length))
...
commonality_score_rand <- unlist(mclapply( gsrand, function(i) {
  jac <- unlist(lapply( gsrand,function(j) {
    myintx <- length(intersect(i,j))
    myunion <- length(union(i,j)) ; myintx/myunion
  } ))
  jac <- jac[order(-jac)]
  mean(jac[2:4])
} , mc.cores=8 ))

names(commonality_score_rand) <- names(gsrand)

hist(commonality_score_rand)

par(mfrow=c(2,1))

hist(commonality_score,xlab="commonality score",main="")
mtext("Reactome pathways")
hist(commonality_score_rand,xlab="commonality score",main="")
mtext("Random gene sets")

par(mfrow=c(1,1))

```

Eliminate pathways with high commonality scores

```{r,eliminateOL1}

length(gsetsf)
length(commonality_score)

gsetsf_common <- gsetsf[which(commonality_score>=0.6)]
gsetsf_unique <- gsetsf[which(commonality_score<0.6)]

length(gsetsf_common)
length(gsetsf_unique)

fres_common_dn <- fres_dn[which(fres_dn$pathway %in% names(gsetsf_common)),]
fres_unique_dn <- fres_dn[which(fres_dn$pathway %in% names(gsetsf_unique)),]
fres_common_up <- fres_up[which(fres_up$pathway %in% names(gsetsf_common)),]
fres_unique_up <- fres_up[which(fres_up$pathway %in% names(gsetsf_unique)),]

fres_common_up_p <- subset(fres_common_up,pval<0.01)$pathway
fres_common_up_fdr <- subset(fres_common_up,padj<0.01)$pathway
fres_common_dn_p <- subset(fres_common_dn,pval<0.01)$pathway
fres_common_dn_fdr <- subset(fres_common_dn,padj<0.01)$pathway

fres_unique_up_p <- subset(fres_unique_up,pval<0.01)$pathway
fres_unique_up_fdr <- subset(fres_unique_up,padj<0.01)$pathway
fres_unique_dn_p <- subset(fres_unique_dn,pval<0.01)$pathway
fres_unique_dn_fdr <- subset(fres_unique_dn,padj<0.01)$pathway

fres_common_p <- unique(c(fres_common_up_p,fres_common_dn_p))
fres_common_fdr <- unique(c(fres_common_up_fdr,fres_common_dn_fdr))
fres_unique_p <- unique(c(fres_unique_up_p,fres_unique_dn_p))
fres_unique_fdr <- unique(c(fres_unique_up_fdr,fres_unique_dn_fdr))

par(mar=c(1,1,1,1))
par(mfrow=c(1,1))
v1 <- list("Common p"=fres_common_p, "Common FDR"=fres_common_fdr)
plot(euler(v1),quantities = list(cex = 2), labels = list(cex = 2))

v1 <- list("Unique p"=fres_unique_p, "Unique FDR"=fres_unique_fdr)
plot(euler(v1),quantities = list(cex = 2), labels = list(cex = 2))

# error rate for common and unique gene sets
length(setdiff(fres_common_p,fres_common_fdr)) / length(fres_common_p)
length(setdiff(fres_unique_p,fres_unique_fdr)) / length(fres_unique_p)

```

Some genes are over-represented in Reactome

```{r,gs_overrep}

par(mfrow=c(2,1))
hist(table(unlist(gsrand)),xlim=c(0,100),breaks=10)
hist(table(unlist(gsets)),xlim=c(0,100),breaks=100)
par(mfrow=c(1,1))
...
Now have a look at the number of genes that are statistically significant below the detection threshold.
Less than 1% of genes are under the detection threshold are significant.
In any case they are unlikely to be detected as significant using qPCR.
...
There is a big difference caused by adding the log2FC filter. At a value of 1.0, genes with middle
expression are over-represented.

At a value of 0.35 it looks better, but middle expressed genes are still over-represented compared to
...
```{r,mistake5,fig.height=6,fig.width=6}
...
```{r,mistake5_linechart,fig.height=7,fig.width=6}
...
Gene list length is 750 due to findings for mistake 5.
The combined approach discovers only a few unique sets.
...
# get the kegg sets from keggrest
#BiocManager::install(c("KEGGREST""EnrichmentBrowser"))
#library(KEGGREST)
#library(EnrichmentBrowser)
#kegg_sets <- getGenesets(org = "hsa", db = "kegg", return.type = "list")
#writeGMT(kegg_sets, gmt.file = "kegg_human_feb2026.gmt")

# translate them to symbols
#library(org.Hs.eg.db)
#library(AnnotationDbi)
#gene.list <- mapIds(org.Hs.eg.db, keys=gene.list[,1], column="SYMBOL", keytype="ENTREZID")
#kegg2 <- gmtPathways("ref/kegg_human_feb2026.gmt")
#entrez <- unique(unlist((kegg2)))
#symbols <- mapIds(org.Hs.eg.db, keys=entrez, column="SYMBOL", keytype="ENTREZID")
#kegg2symbols <- lapply(kegg2,function(x) {
#  unique(unname(symbols[which(entrez %in% x)]))
#})
#writeGMT(kegg2symbols, gmt.file = "ref/kegg_human_feb2026_symbols.gmt")
kegg2symbols <- gmtPathways("ref/kegg_human_feb2026_symbols.gmt")

message("Updated kegg number of sets")
length(kegg2symbols)

message("Updated Kegg total number of annotations")
length(unlist(kegg2symbols))

message("No. genes with one or more annotations")
length(unique(unlist(kegg2symbols)))
...
# kegg2symbols
kegg2_up <- fora(pathways=kegg2symbols, genes=up, universe=bg, minSize = 5, maxSize = Inf)
kegg2_dn <- fora(pathways=kegg2symbols, genes=dn, universe=bg, minSize = 5, maxSize = Inf)
message("no. significantly upregulated KEGG2 pathways")
nrow(subset(kegg2_up,padj<0.01))
message("no. significantly downregulated KEGG2 pathways")
nrow(subset(kegg2_dn,padj<0.01))
message("no. significantly dysregulated KEGG2 pathways")
sum(nrow(subset(kegg2_up,padj<0.01)), nrow(subset(kegg2_dn,padj<0.01) ))
...
kegg_files <- list.files("ref/msigdb/kegg/",full.names=TRUE)
kegg_files <- kegg_files[c(6:20,4:5,1:3)]

keggs <- lapply(kegg_files,gmtPathways)

names(keggs) <- c("2010_09","2012_09","2013_05","2015_04","2016_01","2016_09","2017_04",
  "2017_10","2018_07","2019_08","2020_03","2020_09","2021_03","2021_04","2022_01","2022_09",
  "2023_03","2023_10","2024_08","2025_06")

# analyse kegg 2010 and 2025 - no change
# number of gene sets
nogs <- unlist(lapply(keggs,length))
nogs

# total no annotations
tot_anno <- unlist(lapply(keggs,function(gs) {
  length(unlist(gs))
}))
tot_anno

# gene coverage
gene_coverage <- unlist(lapply(keggs,function(gs) {
  length(unique(unlist(gs)))
}))
gene_coverage

# now analyse Reactome
