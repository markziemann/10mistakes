...
up <- head(rownames(subset(def, log2FoldChange>0 & padj<0.05)),1004)
up <- sapply(strsplit(up," "),'[[',1)
up <- unique(gtnew[match(up,gtnew$V1),2])
up <- up[which(!is.na(up))]

dn <- head(rownames(subset(def, log2FoldChange<0 & padj<0.05)),1001)
...
sigf <- subset(def_unfilt,padj<=0.05)
DET=nrow(def_unfilt)
NSIG=nrow(sigf)
NUP=nrow(subset(sigf,log2FoldChange>0))
NDN=nrow(subset(sigf,log2FoldChange<0))
HEADER=paste(DET,"included genes;",NSIG,"w/FDR<0.05;",NUP,"up;",NDN,"down")
...
sigf <- subset(def_filt,padj<=0.05)
DET=nrow(def_filt)
NSIG=nrow(sigf)
NUP=nrow(subset(sigf,log2FoldChange>0))
NDN=nrow(subset(sigf,log2FoldChange<0))
...
LOWSIG=length( which(def_unfilt$baseMean<10 & def_unfilt$padj < 0.05) )
LOWSIG
...
vres <- lapply(vals, function(i) {
  ...
  abs(y$log2FoldChange)>=1 & y$padj < 0.05
})
...
vres <- lapply(vals, function(i) {
  ...
  y <- def_unfilt[which(log10basemean >= lev0 & log10basemean < lev1 ),c("log2FoldChange","padj")]
  y$padj[is.na(y$padj)] <- 1
  abs(y$log2FoldChange)>=0.35 & y$padj < 0.05
})
...
glup <- head(rownames(subset(def_filt,padj<0.05 & log2FoldChange > 0)),750)
gldn <- head(rownames(subset(def_filt,padj<0.05 & log2FoldChange < 0)),750)
