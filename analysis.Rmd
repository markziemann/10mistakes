...
boxplot(m1sim,main="Random gene sets",ylab="no. gene sets meeting \"significance\" threshold <0.05",frame=FALSE)
...
points(c(0.35,0.8),c(42,8),cex=18,pch=0)
...
points(abs(sig$s.dist),-log10(sig$pANOVA),
  pch=19,cex=1,col="red")
points(c(0.35,0.8),c(41,8),cex=18,pch=0)
...
# get log2fc values
mres_sig_up$median_lfc <- sapply(mres_sig_up$set[1:10], function(y) {
...
mres_sig_dn$median_lfc <- sapply(mres_sig_dn$set[1:10], function(y) {
  genes <- gsets[[which(names(gsets) == y)]]
  gid <- unique(gtnew[match(genes,gtnew$V2),1])
  lfc <- def2[which(rownames(def2) %in% gid),"log2FoldChange"]
  return(median(lfc))
...
# get log2fc values
mres_es_up$median_lfc <- sapply(mres_es_up$set[1:10], function(y) {
  genes <- gsets[[which(names(gsets) == y)]]
  gid <- unique(gtnew[match(genes,gtnew$V2),1])
  lfc <- def2[which(rownames(def2) %in% gid),"log2FoldChange"]
  return(median(lfc))
...
mres_es_dn$median_lfc <- sapply(mres_es_dn$set[1:10], function(y) {
  genes <- gsets[[which(names(gsets) == y)]]
  gid <- unique(gtnew[match(genes,gtnew$V2),1])
  lfc <- def2[which(rownames(def2) %in% gid),"log2FoldChange"]
  return(median(lfc))
...
message("Are median log2FC values for ES prioritisation bigger than FDR approach?")

message("Are median log2FC values for ES prioritisation (UP)")
mean(abs(mres_es_up$median_lfc ))

message("Are median log2FC values for FDR prioritisation (UP)")
mean(abs(mres_sig_up$median_lfc ))

message("ES:FDR ratio (UP)")
mean(abs(mres_es_up$median_lfc )) / mean(abs(mres_sig_up$median_lfc ))

message("Are median log2FC values for ES prioritisation (DOWN)")
mean(abs(mres_es_dn$median_lfc ))

message("Are median log2FC values for FDR prioritisation (DOWN)")
mean(abs(mres_sig_dn$median_lfc ))

message("ES:FDR ratio (DOWN)")
mean(abs(mres_es_dn$median_lfc )) / mean(abs(mres_sig_dn$median_lfc ))
